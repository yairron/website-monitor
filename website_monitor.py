...
if __name__ == "__main__":
    ...
    # בסוף - שלח מייל סיכום יומי (בכל מקרה)
    print("\n" + "="*60)
    print("שליחת דוח סיכום יומי")
    print("="*60)
    
    if monitor.config and monitor.config.get('email', {}).get('enabled', False):
        monitor.send_email_alert(
            subject="✅ דוח יומי - מעקב אתרים הושלם",
            body=f"""דוח מעקב אתרים - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

המערכת בדקה את כל האתרים הבאים:
- עדכון קצבאות
- אזרחים ותיקים  
- קצבת זקנה
- גובה קצבאות 2025
...
אם נמצאו שינויים - נשלחו התראות נפרדות.
...
"""
        ...
        print("⚠️ מיילים לא מופעלים - לא נשלח דוח סיכום")
